...
#python3 train.py --model_name im2ingr --batch_size 32 --finetune_after 0 --ingrs_only --es_metric iou_sample --loss_weight 0 1000.0 1.0 1.0
# --learning_rate 1e-4 --scale_learning_rate_cnn 1.0 --save_dir ../checkpoints --recipe1m_dir ../../../hello/ --transfer_from im2ingr
...
def main(args):
    ...
    for epoch in range(start, args.num_epochs):
                ...
                if args.log_step != -1 and i % args.log_step == 0:
                    ...
                    for k in total_loss_dict.keys():
                        if len(total_loss_dict[k]) == 0:
                            continue
                        lossDict[k].append(np.mean(total_loss_dict[k][-args.log_step:]))
                        print(len(lossDict[k]))
        ...
        lossFile=open("LossFile.pkl","wb")
        pickle.dump(lossDict,lossFile)
        lossFile.close()
